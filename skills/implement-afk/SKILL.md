...
Present the full ordered list of in-scope tickets to the user and get an explicit go-ahead. This is the one checkpoint — after it, the run proceeds unattended.
...
2. **Implement**: trigger a fresh subagent that picks up the ticket cold and runs `/tdd`. The ticket body is its only context — do not hand it conversation history.
3. **Review**: trigger **two reviewer subagents** with split lenses against the branch diff:
...
4. **Address feedback**: the original implementer subagent addresses review feedback over **at most 3 rounds**, re-requesting review each round.
5. **Merge gate** — all of:
